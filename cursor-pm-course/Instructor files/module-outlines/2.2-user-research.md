...
- `lesson-modules/2.2-analyze-data/activation-funnel-q4.csv` *(port from Claude Code course)*
- `lesson-modules/2.2-analyze-data/user-survey-responses.csv` *(port from Claude Code course)*
- `lesson-modules/2.2-analyze-data/taskflow-usage-data-q4.csv` *(port from Claude Code course)*
- `lesson-modules/2.2-analyze-data/onboarding-experiment-results.csv` *(port from Claude Code course)*
- `lesson-modules/2.2-analyze-data/impact-estimation-framework.md` *(port from Claude Code course)*

## Files Created During Module
- `lesson-modules/2.2-analyze-data/activation-problem-analysis.md` (discovery phase findings)
- `lesson-modules/2.2-analyze-data/guided-onboarding-impact-estimate.md` (impact model - realistic scenario)
- `lesson-modules/2.2-analyze-data/guided-onboarding-roi-scenarios.md` (pessimistic, realistic, optimistic scenarios)
- `lesson-modules/2.2-analyze-data/onboarding-experiment-readout.md` (complete A/B test analysis with recommendation)

## High-Level Overview
- **Show overall workflow:** Display Mermaid diagram showing the complete data-driven PM workflow (3 phases: Discovery → Impact Estimation → Experiment Analysis)
- **File orientation:** Explain the CSV files upfront (won't render in Cursor, AI will read and present formatted results), walk through what each dataset contains (funnel, survey, usage, experiment)
- **Phase 1 - Discovery:**
  - Analyze `activation-funnel-q4.csv` to find drop-off point (60% between task creation and completion)
  - **Interactive choice:** "What do you think is causing the 60% drop-off? A) Confusion/overwhelmed, B) Too much friction, C) Missing features, D) Technical issues"
  - Analyze `user-survey-responses.csv` to validate hypothesis (800 responses, extract themes and quotes)
  - **Interactive choice:** "Based on this data, what should we build? A) Task templates, B) Sample project with examples, C) Video tutorial, D) Simplified interface"
  - Cursor creates `activation-problem-analysis.md` synthesizing funnel + survey insights
- **Phase 2 - Impact Estimation:**
  - Review `impact-estimation-framework.md` to learn the formula (Users × Rate × Lift × Value)
  - Analyze `taskflow-usage-data-q4.csv` to build impact model
  - Cursor creates `guided-onboarding-roi-scenarios.md` with three scenarios (pessimistic, realistic, optimistic)
  - **Interactive choice:** "Would you pitch this to leadership? A) Yes, strong ROI, B) Need more data, C) ROI too uncertain"
- **Phase 3 - Experiment Analysis:**
  - Analyze `onboarding-experiment-results.csv` (8,000 users: 4k treatment, 4k control)
  - Show topline results first (modest 45% → 48%, p=0.04)
  - **DRAMATIC MOMENT - Interactive choice:** "Topline looks disappointing. What's your call? A) Ship it anyway, B) Iterate more, C) Kill the feature"
  - THEN reveal segmentation: small teams +11.4pp (p<0.001), enterprise -3.5pp
  - **Interactive choice:** "Now what? A) Ship to everyone, B) Ship to small teams only, C) Kill it, D) Iterate for enterprise"
  - Analyze quality metrics (Week 1 retention 60% → 78%, tasks completed 2.9 → 6.8)
  - Review leading indicators (template usage 3.2x, invite rate 2.9x)
  - Cursor creates `onboarding-experiment-readout.md` with clear recommendation
- **Key teaching:** Cursor makes deep analysis effortless - iteration is free, so you can keep asking questions until you find the insight. What takes hours in spreadsheets takes seconds with Cursor. Show how a "disappointing" topline becomes a huge win when you dig deeper with Cursor.
...
- ACTION: Display Mermaid diagram showing the 3-phase data workflow
	- Phase 1: Discovery (find the problem using funnel + survey data)
	- Phase 2: Impact Estimation (build ROI model to justify fixing it)
	- Phase 3: Experiment Analysis (analyze A/B test results after launch)
...
- STOP: Ask me to say: "@ mention activation-funnel-q4.csv and analyze it to find where users are dropping off in the activation flow"
...
- Here's what I found (I'll show this as a formatted table):
	- Signup: 10,000 users (100%)
	- First Task Created: 7,200 users (72% completion)
	- First Task Completed: 2,880 users (40% completion) ← BIG DROP HERE
	- Invite Sent: 1,440 users (50% completion)
...
- STOP: Ask me to say: "@ mention user-survey-responses.csv and analyze it to extract the top themes explaining why users drop off at task completion"
...
- [If they chose A earlier] Your hypothesis was right - confusion and overwhelm is the issue!
...
- STOP: Ask me to say: "Create activation-problem-analysis.md that synthesizes the funnel data, survey insights, and proposed guided onboarding solution"
...
- ACTION: Create activation-problem-analysis.md synthesizing findings
...
- STOP: Ask me to say: "@ mention impact-estimation-framework.md and explain how to use it for this guided onboarding feature"
...
- STOP: Ask me to say: "@ mention taskflow-usage-data-q4.csv and impact-estimation-framework.md and build three ROI scenarios (pessimistic, realistic, optimistic) for the guided onboarding feature"
...
- ACTION: Create guided-onboarding-roi-scenarios.md with three scenarios
	- Pessimistic (20th percentile): 30% adoption, 45% → 50% activation, +150 users/month, $10k ARR, 2.6x ROI over 3 years
	- Realistic (50th percentile): 70% adoption, 45% → 58% activation, +455 users/month, $39k ARR, 9.4x ROI over 3 years
	- Optimistic (80th percentile): 90% adoption, 45% → 62% activation, +850 users/month, $73k ARR, 17.6x ROI over 3 years
	- Include note: Activated users stay 2.5x longer (historical data), so actual LTV is higher than modeled
...
- Here's what it shows:
	- Even in the pessimistic case: 2.6x ROI over 3 years
	- Realistic case: 9.4x ROI over 3 years
	- Optimistic case: 17.6x ROI
...
- [If C] Understandable caution, but the pessimistic case still shows 2.6x ROI
...
- ACTION: Display sample rows from onboarding-experiment-results.csv as formatted table
...
- STOP: Ask me to say: "@ mention onboarding-experiment-results.csv and analyze it to compare control vs treatment activation rates with statistical significance"
...
- Done! Here are the topline results:
	- Control: 1,808 / 4,000 activated (45.2%)
	- Treatment: 1,912 / 4,000 activated (47.8%)
...
- STOP: Ask me to say: "Segment the experiment results by company size to see if small teams (our target market) had different outcomes than enterprise"
...
- STOP: Ask me to say: "Filter to activated users only and compare Week 1 retention and task completion between control and treatment"
...
- Done! Here's the quality analysis:
	- Control (activated users): 60.1% were active 3+ days in Week 1, average 2.9 tasks completed
	- Treatment (activated users): 78.4% were active 3+ days in Week 1, average 6.8 tasks completed
	- Lift: +18.3 percentage points in retention, 2.3x more tasks completed
	- p < 0.001 (highly significant!)
- Wow! This is huge!
...
- Done! Here are the leading indicators:
	- Template Usage: 10.9% in control vs 35.2% in treatment (3.2x higher, p < 0.001)
		- Users who used templates completed 4.1x more tasks in Week 1
	- Invite Teammate During Onboarding: 12.1% in control vs 34.8% in treatment (2.9x higher, p < 0.001)
...
- STOP: Ask me to say: "Create onboarding-experiment-readout.md with the complete analysis: topline, segmentation, quality metrics, leading indicators, and recommendation"
...
- ACTION: Create onboarding-experiment-readout.md with complete analysis
	- Executive Summary: Ship to small teams (5-20 people), exclude enterprise
	- Topline Results: Modest 45% → 48% overall (p=0.04) but deceiving
	- Segment Analysis: Small teams +11.4pp (p<0.001) HUGE WIN, Enterprise -3.5pp (exclude)
	- Quality Metrics: Week 1 retention +18.3pp (60% → 78%), tasks 2.3x higher
	- Leading Indicators: Template usage 3.2x, invite rate 2.9x
	- Recommendation: Ship to small teams immediately, monitor for 2 weeks, start enterprise discovery
	- Expected Impact: +450 activated users/month, +$39k ARR, 9.4x ROI over 3 years
