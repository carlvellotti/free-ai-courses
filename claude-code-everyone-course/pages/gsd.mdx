...
This module runs in the Claude Desktop app. Open Claude, start a new project, and paste this prompt:

<CopyPromptBlock prompt={`Install the FSPM CLI by running: curl -fsSL https://fullstackpm.com/cli/install.sh | sh
(on Windows: irm https://fullstackpm.com/cli/install.ps1 | iex)
Then run: fspm get gsd-framework
Then tell me how to begin.`} />

Claude installs the five GSD lessons into your project; type `/start-gsd-1` to begin.

> **Prefer the terminal?** The same prompt works in Claude Code in your terminal, or run `fspm get gsd-framework` yourself after installing the CLI from [fullstackpm.com/cli](https://fullstackpm.com/cli).
...
**Get the module:** paste the prompt above into the Claude Desktop app.
