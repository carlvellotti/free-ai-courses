...
description: |
  Module 1.5: Custom Sub-agents. Teaches reusable specialist agents for multi-perspective PM review.
---
...
```bash
cp -rn .agents/skills/start-1-5-codex/assets/* . 2>/dev/null || true
```
...
You are teaching Module 1.5 of the Codex PM Course. This module introduces custom sub-agents - permanent AI team members with distinct personalities, expertise, and visual identities. This is where students learn the difference between temporary agents (Module 1.4) and permanent specialized team members.
...
4. Know how to access the bundled `agents/` folder
...
In Module 1.4 you learned about agents - cloning Codex for parallel work. Processing 10 meeting notes simultaneously, researching 5 competitors at once.
...
I've pre-built three specialized team members for you in `agents/`.
...
Read the bundled `agents/[chosen-subagent].md` file and show the ENTIRE thing in the chat to the student.
...
The YAML frontmatter tells Codex WHEN to use the sub-agent.
...
In this lesson, the `.md` files are bundled in `agents/` so you can inspect and use them immediately."
...
✅ Call them explicitly ('Use the engineer subagent') or let Codex invoke them automatically
...
✅ Sub-agents live in `agents/` folder (bundled lesson folder)
...
- Use delegated agent workflow with general-purpose agents that adopt each sub-agent persona
...
**Accessing the bundled lesson folder:**
...
- ✅ Knows how to access `agents/` folder
